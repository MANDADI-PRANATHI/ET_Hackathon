"""Level 1 entry point: read the corpus and stage source-stamped facts.
  ...
  python scripts/ingest.py [--structured-only] [--ai]
...
def main() -> None:
    ...
    ap.add_argument("--structured-only", action="store_true",
                    help="skip AI prose extraction (Level 0 deps only)")
    ...
    mode = "structured-only" if args.structured_only else "full (AI prose extraction)"
    ...
    if args.path:
        ...
        if use_ai:
            ...
            llm = get_llm()
        staged = ingest_file(pathlib.Path(args.path), corpus, onto, llm=llm)
        if staged is None:
    ...
    counts = ingest_corpus(corpus, staging, onto, use_ai=use_ai)
    print("Done.")
